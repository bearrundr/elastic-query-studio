...
## 2025-04-15 테스트 프레임워크 마이그레이션

### Mocha/Chai에서 Jest로의 테스트 프레임워크 마이그레이션

#### 변경 사항
- Mocha/Chai에서 Jest로 테스트 프레임워크 마이그레이션
- 이중 테스트 모드(목 서버/실제 서버) 구현
- 중앙 집중식 테스트 설정 시스템 생성
- 테스트 파일 문서화 및 헤더 업데이트

#### 테스트 설정
- 중앙 집중식 설정을 위한 `test/test.config.ts` 생성
- 환경 기반 테스트 모드 전환 기능 추가
- 목 응답 템플릿 구현
- 목 데이터용 타입 정의 추가
...
1. `test/setup.ts`
   - Jest 테스트 환경 설정 생성
   - 전역 테스트 설정 및 정리 기능 추가
   - Jest 타임아웃 설정 구성
   - VSCode API 목킹 설정 추가

2. `test/mocks/vscode.ts`
   - VSCode API 목 구현체 생성
   - Sinon 스텁에서 Jest 목 함수로 마이그레이션
   - 타입 안전성이 보장된 목 구현체 추가
   - 목 윈도우 및 문서 처리 기능 강화

3. `test/unit/axios_instance.test.ts`
   - Mocha/Chai에서 Jest 테스트 프레임워크로 변환
   - axios.get 메서드용 Jest 스파이 추가
   - 오류 처리 및 로깅 기능 강화
   - Jest 기대값 스타일로 어설션 업데이트

4. `test/unit/es_connect.test.ts`
   - Jest 프레임워크로 마이그레이션
   - Elasticsearch 응답용 Jest 목 추가
   - 연결 오류 처리 기능 강화
   - 테스트 안정성 개선
